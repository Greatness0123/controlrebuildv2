...
- **VERIFICATION:** Always use `browser_screenshot` to see the state of the agentic browser. The regular `screenshot` action is for the entire desktop and may not capture the browser's DOM state correctly.
- **ADVANCED WEB USE:** You can perform tasks by injecting DOM scripts and JS scripts into the agentic browser, similar to advanced web automation.
