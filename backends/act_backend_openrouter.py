#!/usr/bin/env python3
"""
Control Backend - OpenRouter Variant (X AI Model)
Uses OpenRouter API with X AI free model for GUI automation
"""
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - [CONTROL-OPENROUTER] - %(message)s',
    handlers=[
        logging.StreamHandler(sys.stdout),
        logging.FileHandler('control_openrouter.log')
    ]
)
...
class ComputerUseAgentBackend:
    def __init__(self):
        ...
        logger.info(f"Control Backend (OpenRouter) initialized - Screen: {self.screen_size[0]}x{self.screen_size[1]}")
    ...
    def setup_openrouter_api(self):
        ...
        if not api_key:
            print("[API] ERROR: OPENROUTER_API_KEY not found in environment\n")
        ...
        try:
            ...
            print("[API] OpenRouter API configured\n")
    ...
    def call_openrouter_api(self, prompt: str, screenshot_path: str = None, metadata: Dict[str, Any] = None) -> Dict[str, Any]:
        if not self.api_key:
...
"""
                messages[0]["content"].append({"type": "text", "text": screen_context + prompt})
...
Respond with the JSON TASK structure defined in the system prompt."""
        ...
        initial_response = self.call_openrouter_api(prompt, screenshot_path, metadata)
    ...
    def run_frontend_loop(self) -> None:
        print("\n" + "="*80)
        print(" Control - OpenRouter Backend (Frontend Mode)")
        print("="*80)
